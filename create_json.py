        ...
        try:
            if list_a['ifDescr'] == list_b['ifDescr']:
                ...
                if float(list_b['ifOutOctets']) < float(list_a['ifOutOctets']):
                    ...
                    logging.warning(
                        "OutOctets Exceeding The Maximum Value (-) Prevention (IP:{ip})".format(ip=snmp['ip']))
                else:
                ...
                if float(list_b['ifInOctets']) < float(list_a['ifInOctets']):
                    ...
                    logging.warning(
                        "InOctets Exceeding The Maximum Value (-) Prevention (IP:{ip})".format(ip=snmp['ip']))
                else:
            ...
            logging.error("Json Result Add Fail (IP:{ip} / {detail})".format(ip=snmp['ip'], detail=e))
    try:
